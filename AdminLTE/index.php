...
<?php
	///////// In this php block we handle the submitting of the Form for creating a new List 
	$name_error = "";

	// Check if the form for creating a list was submitted
	if(!empty($_POST['list_btn'])){
		$name = $_POST['name'];


		// Check if the input field for the List Name was submitted empty 
		if(empty($name)){
			// In case of empty filed assing error message to the $name_error variable, and echo it under the input field in the form
			$name_error = "* The list name is required field";
		}
		else {
			// Go to the database and insert the list
			try {
			    // Create Connection
...
<?php
	///////// In this php block we handle the deleting of a single list 
	if(!empty($_POST['delete_list_btn'])){
		$id = $_POST['list_id'];
		try{
			$conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
			$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
...
<?php
	// In this PHP block we get a list of all lists from the database
	try {
	    // Create Connection
	    $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
	    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	    
	    // Create the SQL command for getting all lists from the lists table and use parameters if needed
	    $sql = "SELECT * FROM lists";

	    // Preprate the execution of the SQL command
	    $stmt = $conn->prepare($sql);
      		...
      		<div class="col-xs-12 col-md-6 col-md-offset-3">
      			<div class="box box-info">
			        <div class="box-header with-border">
			          <h3 class="box-title">New List</h3>
			        </div>
			        <!-- /.box-header -->
			        <!-- form start -->
			        <form class="form-horizontal" method="POST" action="index.php">
			          ...
			          <div class="box-footer">
			            <input type="submit" class="btn btn-info pull-right" value="Create List" name="list_btn">
			          </div>
			          <!-- /.box-footer -->
			        </form>
			      </div>
